...
class APIStatsMessageDocTest extends WordSpec with BeforeAndAfterAll with ShouldMatchers {
  ...
  override def beforeAll(): Unit = {
    initLift.boot
    val message1 = new APIStatsMessage("foo", "test1", "", LinkedHashMap("geographyType" -> "block"), LinkedHashMap("latitude" -> "42.456", "longitude" -> "-74.987", "format" -> "json"), new DateTime, 100, true, "", true)
    val message2 = new APIStatsMessage("foo", "test2", "", LinkedHashMap("geographyType" -> "block"), LinkedHashMap("latitude" -> "42.456", "longitude" -> "-74.987", "format" -> "json"), new DateTime, 200, false, "", false)
    val message3 = new APIStatsMessage("foo", "test3", "", LinkedHashMap("geographyType" -> "block"), LinkedHashMap("latitude" -> "42.456", "longitude" -> "-74.987", "format" -> "json"), new DateTime, 175, false, "", true)
